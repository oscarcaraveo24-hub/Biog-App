...
/// Resolver de etapa de la Suculenta (modo `establishment_maintenance`).
///
...
/// así que los contextos viejos se AUTO-REPARAN al abrir la app.
///
/// La suculenta no tiene día terminal: `maintenance` permanece abierto.
class SucculentStageResolver {
  const SucculentStageResolver._();
  ...
  static CropStageResult resolve({
    ...
    final bool hasStoredStage =
        ...
        normalizeSucculentStageId(storedStage) != SucculentStageIds.unknown;

    String stageId;
    double confidence;

    if (hasStoredStage) {
      stageId = normalizeSucculentStageId(storedStage);
      confidence =
          context.ornamentalStageConfidence ??
          succulentStageConfidence(
            stageId: stageId,
            anchorDate:
      ...
      final estimate = resolveSucculentSetupStage(
    ...
    return CropStageResult(
      stageKey: stageId,
      stageLabelEs: succulentStageDisplayName(stageId),
      // Sin día terminal: el mantenimiento no cierra el ciclo.
      expectedDaysToEnd: 0,
      windowsNow: const <dynamic>[],
      heroAsset: SucculentAssets.stageImageOrNeutral(stageId),
  ...
  static String _helperCaption({
    ...
    final parts = <String>[succulentStagePriorityText(stageId)];
