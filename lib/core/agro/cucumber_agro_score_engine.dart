...
/// Motor de score agronómico para pepino.
///
/// Mismo contrato que TomatoAgroScoreEngine: devuelve un AgroEvalResult con
...
/// pasándole cropKey='cucumber'.
///
/// Diferencias agronómicas vs tomate:
/// - EC tiene MÁS peso de penalización (cucurbitácea sensible a salinidad).
/// - Humedad tiene MÁS peso de penalización (planta muy hídrica).
/// - Heat threshold un poco más alto (pepino tolera mejor calor que tomate
///   siempre que tenga humedad), pero por encima de 35 °C cae rendimiento.
/// - Humedad relativa alta sostenida es factor de oídio/mildiu.
class CucumberAgroScoreEngine {
  /// Etapas críticas: cada fallo aquí se paga en cosecha.
  static const Set<CucumberStageKey> _criticalStages = {
    CucumberStageKey.floracion,
    CucumberStageKey.cuajado,
  };

  /// Semi-críticas: sensibles pero con margen de corrección.
  static const Set<CucumberStageKey> _semiCriticalStages = {
    CucumberStageKey.llenado,
    CucumberStageKey.cosechaProgresiva,
  };

  static ({AgroEvalResult eval, AlertsState nextAlertsState}) evaluate({
    required BioGTelemetry t,
    required CucumberStageResult stage,
    required CucumberUniversalProfile u,
    ...
    final nMetric = _interpretCucumberNutrient(
    ...
    final pMetric = _interpretCucumberNutrient(
    ...
    final kMetric = _interpretCucumberNutrient(
    ...
    // Pepino castiga humedad crítica más fuerte que tomate (hídrica).
    ...
    // Pepino más sensible a EC alta sostenida que tomate.
  ...
  static AgroMetricEval _interpretCucumberNutrient({
    required AgroMetricKey metricKey,
    required double rawMgKg,
    required CucumberStageKey stageKey,
  ...
  static void _pushEnvironmentalAlerts(
    ...
    // Pepino MUY sensible a frío: por debajo de 12 °C ya hay parálisis vegetativa.
    if (airTemp <= 0) {
    ...
    // Pepino tolera calor con humedad estable; por encima de 35 °C cae cuajado.
    if (airTemp > 38) {
    ...
    // Humedad relativa alta sostenida favorece oídio/mildiu (cucurbitácea).
    // En invernadero sin ventilación es factor #1 de pérdidas sanitarias.
    if (airHum > 90) {
  ...
  static String _stageLabelEs(CucumberStageKey stageKey) {
    switch (stageKey) {
