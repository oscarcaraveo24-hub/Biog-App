...
class AlertsEngine {
      ...
      case 'ec.critical':
        return _AlertTemplate(
          ...
          body:
              ...
              'puede dañar las raíces. Si es baja, la solución del suelo lleva '
              'pocas sales; no es un diagnóstico de nutrientes.',
        );
  ...
  static _AlertTemplate? _mapKeyToAlert(
    ...
    // Sin plantillas `npk.*`: los motores de score ya no emiten claves
    // nutrimentales (Guía v0.4, §2 y §8). Si una clave vieja llegara desde un
    // estado guardado, cae al final sin alerta, que es la respuesta honesta.
    ...
    if (key == 'ec.critical') {
      ...
      return _AlertTemplate(
        ...
        body:
            ...
            'la salinidad puede dañar las raíces. Si es baja, la solución del '
            'suelo lleva pocas sales; la ventana de nutrición la marca la etapa, '
            'no esta lectura.',
      );
    }
    if (key == 'ec.low') {
      return _AlertTemplate(
        ...
        body:
            'La conductividad es baja para $crop: la solución del suelo lleva '
            'pocas sales. No es un diagnóstico de nutrientes; revisa la tarjeta '
            'de nutrición para ver si la etapa abre ventana.',
      );
    }
    if (key == 'ec.high') {
      if (isLettuce) {
