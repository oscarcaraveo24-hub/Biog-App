...
/// Motor AgroScore del Limón.
///
/// Delega en el motor generico [TreeAgroScoreEngine] (mismo pipeline perenne que
/// manzano/pera/durazno/nogal/pistache/naranjo), resolviendo el modificador del
/// limón y su `cropKey`. No duplica logica base: solo aporta la identidad del
/// cultivo (doc 05 §1, §20). El limón NO es un naranjo pequeño: usa su propio
/// modificador y cap (N=130, P=95, K=210).
class LemonTreeAgroScoreEngine {
  const LemonTreeAgroScoreEngine._();
  ...
  static ({AgroEvalResult eval, AlertsState nextAlertsState}) evaluate({
    ...
    final modifier = resolveLemonTreeNutritionModifier(
      profileId: profileId,
      varietyId: varietyId,
      alias: varietyAlias,
    );
