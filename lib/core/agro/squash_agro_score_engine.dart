...
/// AgroScore específico para calabaza (CropKey.squash).
///
/// Sigue el patrón de berenjena/pepino, con dos diferencias propias del
/// Perfil Universal v2:
///  - Floración y cuajado son ventana CRÍTICA (polinización por insectos
///    define el rendimiento, no solo NPK).
///  - Compactación / anoxia pesa especialmente en establecimiento.
///  - Salinidad es sensibilidad MEDIA, no crítica como en berenjena.
class SquashAgroScoreEngine {
  static const Set<SquashStageKey> _criticalStages = {
    SquashStageKey.floracion,
    SquashStageKey.cuajado,
  };

  static const Set<SquashStageKey> _semiCriticalStages = {
    SquashStageKey.llenado,
    SquashStageKey.cosechaProgresiva,
  };

  static ({AgroEvalResult eval, AlertsState nextAlertsState}) evaluate({
    required BioGTelemetry t,
    required SquashStageResult stage,
    required SquashUniversalProfile u,
    ...
    // Penalizaciones críticas: en flor/cuajado el agua y el calor pesan
    // más que en otras etapas (polinización + amarre).
    double criticalPenalty = 1.0;
    if (moistureEval.band == AgroBand.critical) criticalPenalty *= 0.40;
    if (soilTempEval.band == AgroBand.critical) criticalPenalty *= 0.46;
    if (phEval.band == AgroBand.critical) criticalPenalty *= 0.55;
  ...
  /// Alertas ambientales específicas de calabaza.
  ///
  /// Prioriza: calor en floración (ratio macho/hembra y actividad de
  /// abejas), HR alta/lluvia matinal en floración (riesgo de
  /// polinización fallida) y frío bajo 12 °C.
  static void _pushEnvironmentalAlerts(
    ...
    } else if (airTemp > 35 || (isCriticalStage && airTemp > 32)) {
      out.add('airTemp.heat');
    }

    if (airHum > 92) {
      out.add('airHumidity.critical');
    } else if (airHum > (isCriticalStage ? 80 : 88)) {
      out.add('airHumidity.high');
    }

    // Riesgo de polinización: HR alta + temperatura baja en floración
    // sugiere mañana fría/nublada y poca actividad de abejas.
    if (isCriticalStage && airHum > 85 && airTemp < 22) {
      out.add('squash.pollination_risk');
    }

    // En CA-07 (pipián), el calor extremo durante llenado de semilla
    // afecta cuajado y peso de pepita.
    if (isSeedFocused &&
        stage.stage == SquashStageKey.llenado &&
        airTemp > 35) {
      out.add('squash.seed_heat_risk');
    }
  }
  ...
  static String _stageLabelEs(SquashStageKey stageKey) {
    switch (stageKey) {
