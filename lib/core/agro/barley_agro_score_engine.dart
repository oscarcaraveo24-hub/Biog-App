...
class BarleyAgroScoreEngine {
  /// Etapas críticas para cebada: embuche, espigado y floración.
  static const Set<BarleyStageKey> _criticalStages = {
    BarleyStageKey.booting,
    BarleyStageKey.heading,
    BarleyStageKey.flowering,
  };

  static const Set<BarleyStageKey> _semiCriticalStages = {
    BarleyStageKey.grainFill,
  };
  ...
  static ({AgroEvalResult eval, AlertsState nextAlertsState}) evaluate({
    ...
    final nMetric = _interpretBarleyNutrient(
    ...
    final pMetric = _interpretBarleyNutrient(
    ...
    final kMetric = _interpretBarleyNutrient(
  ...
  static AgroMetricEval _interpretBarleyNutrient({
    required AgroMetricKey metricKey,
    required double rawMgKg,
    required BarleyStageKey stageKey,
  ...
  static void _pushLegacyAlertsForMetric(
    ...
    String key,
    _Eval e,
    BarleyStageKey stage,
  ) {
    final isCriticalStage = _criticalStages.contains(stage) || _semiCriticalStages.contains(stage);
  ...
  static void _pushNutrientAlertsForMetric(
    ...
    switch (label) {
      ...
      case NutrientPriorityLabel.reviewAccumulation:
        ...
        return;
      case NutrientPriorityLabel.mediumPriority:
        if (_criticalStages.contains(stage) || _semiCriticalStages.contains(stage)) {
  ...
  static void _pushEnvironmentalAlerts(
    ...
    } else if (airTemp > 30 || (isCriticalStage && airTemp > 28)) {
  ...
  static String _stageLabelEs(BarleyStageKey stageKey) {
    switch (stageKey) {
